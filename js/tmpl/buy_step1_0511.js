$(function() {
	...
	$.ajax({//提交订单信息
		...
		success:function(result){
						...
						$.each(v.goods_list,function(k1,v1){//循环商品列表
							...
							htmldata+='<div class="clearfix">'
												...
												+'<div class="buys1-pdlcnt">'
													...
													+'<p>应付款(元)：￥'+v1.goods_price+'　　<span>数量：'+v1.goods_num+'</span></p>'
													+'<p>债权金(元)：￥'+v1.goods_price_zq+'</p>'
												+'</div>'
											+'</div>'
										+'</div>';
						});
			...
			if (data.available_rc_balance != null && data.available_rc_balance > 0) { // 债权金余额
				$('.pre-deposit-wp').show();
...
*/
        ...
        if (available_rc_balance > 0 && $('#usercbpay').prop('checked')) { // 使用债权金
