...
var hui = (function(selector, undefined){
	...
	var hcExtends = {
		...
		longTap : function(callBack){
			...
			this.dom[0].addEventListener('touchstart',function(){
				timer = setInterval(function(){if(timerNum >= 1500){_self.trigger('longTapDo'); timerNum = 0; clearInterval(timer);}else{timerNum += 100;}},100);
			});
		...
		getSwitchVal : function(){if(this.hasClass('hui-switch-on')){return true;} return false;},
	...
	huibase.Back   = function(){
		...
		if(selfW.id == plus.runtime.appid || selfW.id == 'HBuilder'){
			...
			if(selfW.huiIsClose){if(selfW.huiIsClose == 'no'){return;} selfW.close('slide-out-right');}else{selfW.hide('slide-out-right');}
		}
	};
	...
	huibase.confirm = function(msg, btnName, callBack){
		...
		btns[1].onclick = function(){hui.dialogClose(); callBack();};
	};
